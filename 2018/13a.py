...
def solve(inp):
    ...
    while not part1:
        ...
        for i in range(len(carts)):
            if moveCart(i, turns, carts, grid):
                part1 = carts[i][:2]
                break
            #print(carts)
    part2 = 0
...
def moveCart(i, turns, carts, grid):
    crash = False
    if (carts[i][0]+carts[i][2],carts[i][1]+carts[i][3]) in set(tuple(carts[i][:2]) for i in range(len(carts))):
        crash = True
    x, y, dx, dy = carts[i]
    #print(carts[i])
    x += dx
